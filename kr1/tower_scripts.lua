...
local function register_archer(scripts)
    -- 矮人射手
            ...
            while true do
                if this.tower.blocked then
                    ...
                    if ready_to_attack(aa, store, this.tower.cooldown_factor) then
                        local enemy, enemies = U.find_foremost_enemy_with_flying_preference(store.enemies, tpos(this),
                            0, a.range, false, aa.vis_flags, aa.vis_bans)
            ...
            while true do
                if this.tower.blocked then
                    ...
                    if ready_to_use_power(pow_m, ma, store, this.tower.cooldown_factor) then
                        local enemy = U.find_foremost_enemy_with_flying_preference(store.enemies, tpos(this), 0,
                            a.range, false, ma.vis_flags, ma.vis_bans)
                        ...
                        if not enemy then
                            ...
                            local an, af = U.animation_name_facing_point(this, "multishot_start", enemy.pos,
                                shooter_sid, start_offset)
                            ...
                            for i = 1, ma.shots + pow_m.level * ma.shots_inc do
                                ...
                                if last_enemy.health.dead then
                                    enemy = U.find_foremost_enemy_with_flying_preference(store.enemies, origin, 0,
                                        range, false, ma.vis_flags, ma.vis_bans)
                                end
                            ...
                            local an, af = U.animation_name_facing_point(this, "multishot_end", last_enemy.pos,
                                shooter_sid, start_offset)
                    ...
                    if ready_to_attack(aa, store, this.tower.cooldown_factor) then
                        ...
                        if not enemy then
                            ...
                            local an, af = U.animation_name_facing_point(this, "shoot", enemy.pos, shooter_sid,
                                start_offset)
                            ...
                            an, af = U.animation_name_facing_point(this, "idle", last_target_pos, shooter_sid,
                                start_offset)
            ...
            while true do
                if this.tower.blocked then
                    ...
                    if ready_to_attack(a, store, this.tower.cooldown_factor) then
                        local enemy, _, pred_pos = U.find_foremost_enemy_with_flying_preference(store.enemies,
                            tpos(this), 0, at.range, a.node_prediction, a.vis_flags, a.vis_bans)
                ...
                if this.tower.blocked then
                    ...
                    for k, pow in pairs(this.powers) do
                        if pow.changed then
                            ...
                            if k == "mark" then
                                this.attacks.list[3].cooldown =
                                    this.attacks.list[3].cooldown + this.attacks.list[3].cooldown_inc
                            end
                        end
                    end
                    SU.tower_update_silenced_powers(store, this)
...
local function register_mage(scripts)
    -- 狂野魔术师
                ...
                if this.tower.blocked then
                    ...
                    if ready_to_use_power(pow_e, ea, store, this.tower.cooldown_factor) then
                        ...
                        if not enemy then
                            ...
                            local an, af, ai = U.animation_name_facing_point(this, ea.animation, enemy.pos, shooter_sid,
                                so)
                    ...
                    if ready_to_use_power(pow_w, wa, store, this.tower.cooldown_factor) then
                        ...
                        if enemy then
                            ...
                            local so = this.render.sprites[shooter_sid].offset
                            local an, af, ai = U.animation_name_facing_point(this, wa.animation, enemy.pos, shooter_sid,
                                so)
                    ...
                    if store.tick_ts - ba.ts > this.tower.long_idle_cooldown then
                        local an, af =
                            U.animation_name_facing_point(this, "idle", this.tower.long_idle_pos, shooter_sid)
    ...
    scripts.tower_arcane_wizard = {
        ...
        update = function(this, store)
                    ...
                    aa.node_prediction, aa.vis_flags, aa.vis_bans, function(e)
                        if aa == at then
                            return e.nav_path.ni >= aa.min_nodes and
                                       (not e.enemy.counts.mod_teleport or e.enemy.counts.mod_teleport <
                                           max_times_applied)
                        else
                            return true
                        end
            ...
            local function start_animations(attack, enemy)
                ...
                local an, af, ai =
                    U.animation_name_facing_point(this, attack.animation, enemy.pos, shooter_sid, soffset)
            ...
            local function wizard_attack(attack, enemy, pred_pos)
                ...
                if attack == at then
                    ...
                    b.pos.x, b.pos.y = this.pos.x + attack.bullet_start_offset.x,
                        this.pos.y + attack.bullet_start_offset.y
                    b.bullet.from = V.vclone(b.pos)
            ...
            while true do
                ...
                if ((ad.ts <= last_ts - (ad.cooldown - a.min_cooldown) * this.tower.cooldown_factor) or
                    (store.tick_ts - ad.ts >= (ad.cooldown - a.min_cooldown) * this.tower.cooldown_factor)) and
                    pow_d.level > 0 then
                    ...
                    for i, aa in pairs(attacks) do
                        ...
                        if (pow and ready_to_use_power(pow, aa, store, this.tower.cooldown_factor)) or
                            ...
                            if not enemy then
                                ...
                                local soffset = this.render.sprites[shooter_sid].offset
                                local an, af, ai = U.animation_name_facing_point(this, aa.animation, enemy.pos,
                                    shooter_sid, soffset)
                ...
                if this.tower.blocked then
                    ...
                    if ready_to_attack(ba, store, this.tower.cooldown_factor) then
                        ...
                        if not target and (not ba.max_stored_bullets or ba.max_stored_bullets == #this._stored_bullets) then
                            ...
                            if target then
                                local tx, ty = V.sub(target.pos.x, target.pos.y, this.pos.x,
                                    this.pos.y + s_tower.offset.y)
                    ...
                    if pow_r.level > 0 then
                        if pow_r.changed then
                            ...
                            if s and store.entities[s.id] then
                                ...
                                s.health.hp_max = s.health.hp_max + s.health.hp_inc
                                s.health.armor = s.health.armor + s.health.armor_inc
                                s.melee.attacks[1].damage_min =
                                    s.melee.attacks[1].damage_min + s.melee.attacks[1].damage_inc
                                s.melee.attacks[1].damage_max =
                        ...
                        if not s or s.health.dead and store.tick_ts - s.health.death_ts > s.health.dead_lifetime then
                            ...
                            s.melee.attacks[1].damage_min = s.melee.attacks[1].damage_min +
                                                                s.melee.attacks[1].damage_inc * s.unit.level
            ...
            if this.tower.blocked then
                ...
                if store.tick_ts - a.ts > a.cooldown * this.tower.cooldown_factor then
                    for _, pixie in pairs(pixies) do
                        ...
                        if pixie.target or store.tick_ts - pixie.attack_ts <= a.pixie_cooldown *
                            ...
                            if not trigger then
                                ...
                                if V.dist(tpos(this).x, tpos(this).y, dest.x, dest.y) <= aa.range then
                                    ...
                                    if aa == am then
                                        ...
                                        if aa == ac then
                                            b.bullet.fragment_count = pow_c.fragment_count_base +
                                                                          pow_c.fragment_count_inc * pow_c.level
                                        end
                                    end
                            ...
                            for i = 1, #fx_points do
                                ...
                                if band(p.terrain, TERRAIN_WATER) ~= 0 then
                                    ...
                                    if lava_ready then
                                        ...
                                        vapor.pos.x, vapor.pos.y = p.pos.x + U.frandom(-5, 5),
                                            p.pos.y + U.frandom(-5, 5)
                        ...
                        if triggers and #triggers > ca.min_count then
                            ...
                            if targets then
                                for i, target in ipairs(targets) do
                                    ...
                                    if band(target.vis.bans, F_STUN) == 0 and
                                        band(target.vis.flags, bor(F_BOSS, F_FLYING)) == 0 and chance > math.random() then
                    ...
                    if not targets then
                        ...
                        for i = 1, fire_loops do
                            ...
                            for hi, ht in ipairs(am.hit_times) do
                                ...
                                b.pos.x = this.pos.x + (af and -1 or 1) *
                                              am.start_offsets[km.zmod(hi, #am.start_offsets)].x
                    ...
                    if ready_to_attack(ra, store, this.tower.cooldown_factor) then
                        ...
                        if not enemy then
                            ...
                            if not enemy or store.entities[enemy.id] == nil or enemy.health.dead or
                                ...
                                enemy = U.find_foremost_enemy(store.enemies, tpos(this), 0, at.range,
                                    ra.node_prediction, ra.vis_flags, ra.vis_bans)
                            end
...
return function(scripts)
    register_archer(scripts)
    register_mage(scripts)
    register_engineer(scripts)
    register_barrack(scripts)
end
